...
func TestConfirm_MediumRisk_Y(t *testing.T) {
	m := New(testRb(domain.RiskMedium), testCat(), map[string]string{}, 60, nil)
...
func TestConfirm_MediumRisk_N(t *testing.T) {
	m := New(testRb(domain.RiskMedium), testCat(), map[string]string{}, 60, nil)
...
func TestConfirm_HighRisk_TypeID(t *testing.T) {
	rb := testRb(domain.RiskHigh)
...
func TestConfirm_HighRisk_WrongID(t *testing.T) {
...
func TestConfirm_CriticalRisk_TypeCONFIRM(t *testing.T) {
	m := New(testRb(domain.RiskCritical), testCat(), map[string]string{}, 60, nil)

	for _, ch := range "CONFIRM" {
		m, _ = m.Update(tea.KeyPressMsg{Code: ch, Text: string(ch)})
	}

	m, cmd := m.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	if cmd == nil {
		t.Fatal("enter after typing CONFIRM should produce a command")
	}
	msg := cmd()
	if _, ok := msg.(ConfirmAcceptMsg); !ok {
		t.Fatalf("expected ConfirmAcceptMsg, got %T", msg)
